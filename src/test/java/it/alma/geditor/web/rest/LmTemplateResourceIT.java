...
public class LmTemplateResourceIT {
    ...
    @Mock
    private LmTemplateRepository lmTemplateRepositoryMock;
    ...
    public void getAllLmTemplatesWithEagerRelationshipsIsEnabled() throws Exception {
        LmTemplateResource lmTemplateResource = new LmTemplateResource(lmTemplateRepositoryMock);
        when(lmTemplateRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restLmTemplateMockMvc = MockMvcBuilders.standaloneSetup(lmTemplateResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restLmTemplateMockMvc.perform(get("/api/lm-templates?eagerload=true"))
        .andExpect(status().isOk());

        verify(lmTemplateRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllLmTemplatesWithEagerRelationshipsIsNotEnabled() throws Exception {
        LmTemplateResource lmTemplateResource = new LmTemplateResource(lmTemplateRepositoryMock);
            when(lmTemplateRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restLmTemplateMockMvc = MockMvcBuilders.standaloneSetup(lmTemplateResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
