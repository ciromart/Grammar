...
public class ModelResourceIT {
    ...
    public void getAllModelsWithEagerRelationshipsIsEnabled() throws Exception {
        ModelResource modelResource = new ModelResource(modelRepositoryMock);
        when(modelRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restModelMockMvc = MockMvcBuilders.standaloneSetup(modelResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restModelMockMvc.perform(get("/api/models?eagerload=true"))
        .andExpect(status().isOk());
    ...
    @SuppressWarnings({"unchecked"})
    public void getAllModelsWithEagerRelationshipsIsNotEnabled() throws Exception {
        ModelResource modelResource = new ModelResource(modelRepositoryMock);
            when(modelRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restModelMockMvc = MockMvcBuilders.standaloneSetup(modelResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
